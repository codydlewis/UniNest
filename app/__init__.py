...
def home():
    ...
    if form.validate_on_submit():
        ...
        rent = get_rent_values(multiplier=form.acc_prio.data, rent_number=form.rent.data)
        user_input.update(rent)
        #put user input inside model and get output here
        output = get_suburb_from_prefs(user_input)
        return redirect(url_for('map', output=output))
...
def map():
    ...
    return render_template("map.html", title="Map", form=form, data=suburb_name)
